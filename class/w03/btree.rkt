...
(list->tree '(5 4 2 9 6 10))
(define x (list->tree '(5 4 2 9 6 10)))
...
(define tree-map
  (lambda (fn tree)
    (cond ((empty-tree? tree) tree)
          (else (make-tree (fn (get-data tree))
...
(define count-leaves
  (lambda (tree)
    (cond ((empty-tree? tree) 0)
          ...
          (else (+ (count-leaves (get-left tree)) (count-leaves (get-right tree)))))))
...
(define tree-depth
  (lambda (tree)
    (cond ((empty-tree? tree) 0)
          ((and (empty-tree? (get-left tree))
                (empty-tree? (get-right tree))) 1)
          (else (max (add1 (tree-depth (get-left tree))) 
                     (add1 (tree-depth (get-right tree))))))))
...
(define tree-accumulate
  (lambda (tree combiner base term)
    (cond ((empty-tree? tree) base)
          (else (combiner (term (get-data tree))
                          (tree-accumulate (get-left tree) combiner base term)
                          (tree-accumulate (get-right tree) combiner base term))))))


; adds all elts in tree
(tree-accumulate x + 0 identity)  ; 36

; multiplies all elts in tree
(tree-accumulate x * 1 identity)  ; 21600
