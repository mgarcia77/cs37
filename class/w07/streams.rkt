...
(define primesums
  (lambda (n)
    (define helper
      (lambda (n p result)
        (cond ((<= n 0) result)
              ((prime? p) (helper (sub1 n) (add1 p) (+ result p)))
              (else (helper n (add1 p) result)))))
    (helper n 2 0)))

;; method 2
(foldl + 0 (first 10 (filter prime? (enumerate 1 100))))

;; method 3
(foldl + 0 (sfirst 10 (sfilter prime? (senumerate 1 100000))))

(define ps2
  (lambda (n)
    (foldl + 0 (sfirst n (sfilter prime? positives)))))

;;;;;;;;;;
(set! scdr
  (lambda (s)
    (let ((next (force-expression (mcdr s))))
      (set-mcdr! s (delay-expression next))  ;; want to cache a stream value that's already calculated
      next)))  ;; this version stores a lot of extra procedures

(set! scdr
  (lambda (s)
    (cond ((procedure? (mcdr s))  ;; relatively expensive check
           (let ((next (force-expression (mcdr s))))
             (set-mcdr! s next)  ;; want to cache but without additional procedure wrap
             next))
          (else(mcdr s)))))
