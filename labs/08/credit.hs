...
-- Exercise 1. Define these functions to convert an Integer
...
toDigits num
  | num <= 0 = []
  | otherwise = (toDigits (div num 10)) ++ (mod num 10) : []
...
doubleEveryOther = reverse . doubleReversed . reverse
  where doubleReversed [] = []
        doubleReversed lst@(x:[]) = lst
        doubleReversed (x:y:zs) = x : (2 * y) : (doubleReversed zs)
